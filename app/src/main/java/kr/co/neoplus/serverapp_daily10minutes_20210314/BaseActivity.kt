...
//    다른 화면들에서도 커스텀액션바에 달아둔 UI 요소들을 사용해야할 경우가 있다
//    멤버변수로 UI 요소를 만들어둬야 물려받아서 사용 가능
//    만드는건 멤버변수 > 대입은 커스텀액션바 완성 이후에 가능 (나중에 가능)
...
//  이 클래스에서 onCreate 의 내용을 수정 (오버라이딩)하면
//  자녀들이 가진 (모든 화면에서 가진) super.onCreate 에서 동작됨

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

//    모든 화면은 만들어질 때 액션바 커스터마이징 실행
//    액션바가 존재하는 화면에서만 => 커스터마이징도 진행

        supportActionBar?.let {

//            기본 액션바가 실제로 존재할 때 (let 의 역할)만, 커스터마이징 진행
            setCustomActionBar()
        }
...
//    모든 화면에 공통적인 기능을 만들 때는 여기서 처리하자
